...
local function noobhaxxKill(model)
	if not model then return end
	local plr = game.Players:GetPlayerFromCharacter(model)
	if plr and plr ~= me then
		local revengeKill = game.ReplicatedStorage:FindFirstChild("revengeKill")
		if revengeKill then
			revengeKill:FireServer(unpack({plr.Name}))
		end
	end
end
